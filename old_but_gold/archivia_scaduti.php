...
$dati_canc = $conn->query("SELECT email_notifiche,codice FROM acs_pacchetti WHERE cestinato != '1' && ( data_fine_pacchetto <= curdate() || ore_utilizzate >= (ore_totali_pacchetto + delta_ore))");

if ($dati_canc->num_rows>0) {
...
$conn->query("DELETE FROM acs_pacchetti WHERE cestinato != '1' && ( data_fine_pacchetto <= curdate() || ore_utilizzate >= (ore_totali_pacchetto + delta_ore))");
