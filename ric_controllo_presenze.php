	...
	if ( $_POST['al'] !== '' ) { $al = $_POST["al"]; } else { $al =  date('Y-m-d',strtotime("last day of this month")); }
		...
		echo "<table id=customers><tr><thead><th colspan = 4>ACCESSI PERIODO (dal ".date('d-M-Y', strtotime($dal))." al ".date('d-M-Y', strtotime($al)).")</th></tr>"
                ...
                $q_nomi_sing = $conn->query("select distinct(nome_azienda) as dipendente, pin, auth from union_sums "
                        . " where nome_azienda !=  '' and nome_azienda not like '%MNTN%' and nome_azienda not like '%fornitor%' and data_ingresso between '".$dal."' and '".$al."' and pin like '".$pin."' "
...
if (isset($_POST["button"])) { echo "<BR><P align='center'><a href=\"#\" onclick=\"$('#customers').tableExport({type:'excel',escape:'false'});\"><img src='images/export_xls.png' width=64 title='ESPORTA IN EXCEL'></a><a href=\"#\" onclick=\"stampa()\"><img src='images/export_print.png' width=64 title='STAMPA'></a></p>"; }
