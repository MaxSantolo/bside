...
body {
  ...
  background-image: url(images/sfondo_fornitori.jpg);
...
<div class="hit-the-floor">Visualizzazione accessi - FORNITORI</div></h3><BR>
...
//                break;
         ...
         $sql = 
                  "SELECT * FROM cdr_accessi_sum WHERE (nome_azienda LIKE '%".$azienda."%' and nome_azienda LIKE '%fornit%') && pin LIKE '%".$codice."%' && (data_ingresso BETWEEN '".$dal."' AND '".$al."') AND src ".$intcit." "
                . "     UNION ALL "
                . "SELECT * FROM cdr_accessi_eur_sum WHERE (nome_azienda LIKE '%".$azienda."%' and nome_azienda LIKE '%fornit%') && pin LIKE '%".$codice."%' && (data_ingresso BETWEEN '".$dal."' AND '".$al."') AND src  ".$intcit." "
                 . "     UNION ALL "
                . "SELECT * FROM cdr_accessi_regolo_sum WHERE (nome_azienda LIKE '%".$azienda."%' and nome_azienda LIKE '%fornit%') && pin LIKE '%".$codice."%' && (data_ingresso BETWEEN '".$dal."' AND '".$al."') AND src ".$intcit." "
                . "     ORDER BY pin ASC, data_ingresso ASC, nome_azienda ASC";
        
} else { $sql = 
                  "SELECT * FROM cdr_accessi_sum WHERE (nome_azienda LIKE '%fornit%') AND (data_ingresso BETWEEN '".date("Y-m-d", strtotime("first day of this month"))."' AND '".date("Y-m-d", strtotime("last day of this month"))."') && auth LIKE '95'"
                . "     UNION ALL "
                . "SELECT * FROM cdr_accessi_eur_sum WHERE (nome_azienda LIKE '%fornit%') AND (data_ingresso BETWEEN '".date("Y-m-d", strtotime("first day of this month"))."' AND '".date("Y-m-d", strtotime("last day of this month"))."') && auth LIKE '95'"
                . "     UNION ALL "
                . "SELECT * FROM cdr_accessi_regolo_sum WHERE (nome_azienda LIKE '%fornit%') AND (data_ingresso BETWEEN '".date("Y-m-d", strtotime("first day of this month"))."' AND '".date("Y-m-d", strtotime("last day of this month"))."') && auth LIKE '95'"
                . "     ORDER BY pin ASC, data_ingresso ASC, nome_azienda ASC"; }

$citofoni_boezio = array('3531','3532','3533');
$citofoni_eur = array('3831','3832','3833','3834','3534');
	...
	$num_righe = $result->num_rows;
		...
		while($row = $result->fetch_assoc()) {
                ...
                echo "<tr><td>".$row["nome_azienda"]."</td><td>".$datifop["firstname"]."</td><td>".$datifop["lastname"]."</td><td>".date("l d/m/Y", strtotime($row["data_ingresso"]))."</td><td width=\"500\">".$row["ingressi"]."</td><td>".$row["pin"]."</td><td>".$row["auth"]."</td><td>".$sede."</td>"
                        ...
                        ."<td  width='24'><A HREF='edit_nota.php?pin=".$row["pin"]."&data=".$row["data_ingresso"]."&centro=".$sede."&nota=".$note_accesso["nota"]."&ric=fornitori'><IMG SRC='../images/file_edit.png' width='24' title='MODIFICA/INSERISCI NOTA'></a></TD>";
		}
